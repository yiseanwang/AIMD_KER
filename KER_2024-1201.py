...
def kinetic_energy_final(xyz_file, the_frag, masses):
	...
	i=0
	while i < len(the_frag):
		atom = lines[int(the_frag[i]+2)].split()  # 把第the_frag[i], 第i個原子,行分開 [x, y, z, vx, vy, vz]
		vix, viy, viz = float(atom[-3]), float(atom[-2]), float(atom[-1])
		Vcx =  Vcx + masses[int(the_frag[i])]*(vix/M)   
		Vcy =  Vcy + masses[int(the_frag[i])]*(viy/M)
		Vcz =  Vcz + masses[int(the_frag[i])]*(viz/M)
		i=i+1
	final_K = 0.5 * 104.208 * M * (Vcx**2+Vcy**2+Vcz**2)
	return np.ravel(final_K)
#--------------------------------
...
datafile = open("bond_length.csv","w+") 
...
while i < len(df.index): 
    r = df.iloc[i]        #r代表row的意思,這樣寫下一行比較短，或許下面改成function
    print(r)
## H2O+ H2O+
    if (r[' 0-3'] > 10) & (r[' 0-1'] < 2) & (r[' 0-2'] < 2) & (r[' 3-4'] < 2)  & (r[' 3-5'] < 2):
        df.at[i,'type']= "type1"
        frag1=[0,1,2]
        frag2=[3,4,5]
        print(str(r['filename'])+"_final.xyz")
        KER_1=kinetic_energy_final(str(r['filename'])+"_final.xyz", frag1, masses)       # BOMD最後 frag1 
        KER_2=kinetic_energy_final(str(r['filename'])+"_final.xyz", frag2, masses)       # BOMD最後 frag2
        OandO=[[0,3]]
        V_1=potential_enegy(str(r['filename'])+ "_init.xyz", top_file_path, charge1, charge2, OandO)  # Ehrenfest最後 
        V_0=potential_enegy(str(r['filename'])+"_Eh0fs.xyz", top_file_path, charge1, charge2, OandO) # Ehrenfest開始
        df.at[i,'KER']=round((KER_1+KER_2+(V_0-V_1)).flat[0],3)
## H3O+ OH+
    elif (r[' 0-3'] > 10) & (r[' 0-1'] < 2) & (r[' 0-2'] < 2) & (r[' 0-5'] < 2)  & (r[' 3-4'] < 2):
        df.at[i,'type']= "type2-a"
        frag1=[0,1,2,5]
        frag2=[3,4]
        KER_1=kinetic_energy_final(str(r['filename'])+"_final.xyz", frag1, masses)       # BOMD最後 frag1 
        KER_2=kinetic_energy_final(str(r['filename'])+"_final.xyz", frag2, masses)       # BOMD最後 frag2
        ...
        V_0=potential_enegy(str(r['filename'])+"_Eh0fs.xyz", top_file_path, charge1, charge2, OandO) # Ehrenfest開始
        df.at[i,'KER']=round((KER_1+KER_2+(V_0-V_1)).flat[0],3) 
    elif (r[' 0-3'] > 10) & (r[' 0-1'] < 2) & (r[' 0-2'] < 2) & (r[' 0-4'] < 2)  & (r[' 3-5'] < 2):
        df.at[i,'type']= "type2-b"
        frag1=[0,1,2,4]
        frag2=[3,5]
        KER_1=kinetic_energy_final(str(r['filename'])+"_final.xyz", frag1, masses)       # BOMD最後 frag1 
        KER_2=kinetic_energy_final(str(r['filename'])+"_final.xyz", frag2, masses)       # BOMD最後 frag2
        ...
        V_0=potential_enegy(str(r['filename'])+"_Eh0fs.xyz", top_file_path, charge1, charge2, OandO) # Ehrenfest開始
        df.at[i,'KER']=round((KER_1+KER_2+(V_0-V_1)).flat[0],3)
    elif (r[' 0-3'] > 10) & (r[' 0-1'] < 2) & (r[' 2-3'] < 2) & (r[' 3-4'] < 2)  & (r[' 3-5'] < 2):
        df.at[i,'type']= "type2-c"
        frag1=[0,1]
        frag2=[2,3,4,5]
        KER_1=kinetic_energy_final(str(r['filename'])+"_final.xyz", frag1, masses)       # BOMD最後 frag1 
        KER_2=kinetic_energy_final(str(r['filename'])+"_final.xyz", frag2, masses)       # BOMD最後 frag2
        ...
        V_0=potential_enegy(str(r['filename'])+"_Eh0fs.xyz", top_file_path, charge1, charge2, OandO) # Ehrenfest開始
        df.at[i,'KER']=round((KER_1+KER_2+(V_0-V_1)).flat[0],3)
    elif (r[' 0-3'] > 10) & (r[' 0-2'] < 2) & (r[' 1-3'] < 2) & (r[' 3-4'] < 2)  & (r[' 3-5'] < 2):
        df.at[i,'type']= "type2-d"
        frag1=[0,2]
        frag2=[3,1,4,5]
        KER_1=kinetic_energy_final(str(r['filename'])+"_final.xyz", frag1, masses)       # BOMD最後 frag1 
        KER_2=kinetic_energy_final(str(r['filename'])+"_final.xyz", frag2, masses)       # BOMD最後 frag2
...
#                 #O1-O4              #O1-H2           #O1-H3             #O4-H5             #O4-H6          # O1-H6
    elif (r[' 0-3'] > 10) & (r[' 0-1'] < 2) & (r[' 0-2'] < 2) & (r[' 3-4'] < 2)  & (r[' 3-5'] >3) & (r[' 0-5'] >3):
        df.at[i,'type']= "type3-a"
        frag1=[0,1,2]
        frag2=[3,4]
        frag3=[5]
        KER_1=kinetic_energy_final(str(r['filename'])+"_final.xyz", frag1, masses)       # BOMD最後 frag1 
        KER_2=kinetic_energy_final(str(r['filename'])+"_final.xyz", frag2, masses)       # BOMD最後 frag2
        KER_3=kinetic_energy_final(str(r['filename'])+"_final.xyz", frag3, masses)
        OandO=[0,3]
        V_1=potential_enegy(str(r['filename'])+ "_init.xyz", top_file_path, charge1, charge2, OandO)  # Ehrenfest最後 
        V_0=potential_enegy(str(r['filename'])+"_Eh0fs.xyz", top_file_path, charge1, charge2, OandO) # Ehrenfest開始
        df.at[i,'KER']=round((KER_1+KER_2+KER_3+(V_0-V_1)).flat[0],3)
    elif (r[' 0-3'] > 10) & (r[' 0-1'] < 2) & (r[' 0-2'] < 2) & (r[' 3-5'] < 2)  & (r[' 3-4'] >3) & (r[' 0-4'] >3):
        df.at[i,'type']= "type3-b"
        frag1=[0,1,2]
        frag2=[3,5]
        frag3=[4]
        KER_1=kinetic_energy_final(str(r['filename'])+"_final.xyz", frag1, masses)       # BOMD最後 frag1 
...
#                 #O1-O4              #O4-H5            #O4-H6             #O1-H2          #O1-H3          # O1-H6
    elif (r[' 0-3'] > 10) & (r[' 3-4'] < 2) & (r[' 3-5'] < 2) & (r[' 0-1'] < 2)  & (r[' 0-2'] >3) & (r[' 2-3'] >3):
        df.at[i,'type']= "type3-c"
        frag1=[0,1]
        frag2=[3,4,5]
        frag3=[2]
        KER_1=kinetic_energy_final(str(r['filename'])+"_final.xyz", frag1, masses)       # BOMD最後 frag1 
        ...
        V_0=potential_enegy(str(r['filename'])+"_Eh0fs.xyz", top_file_path, charge1, charge2, OandO) # Ehrenfest開始
        df.at[i,'KER']=round((KER_1+KER_2+KER_3+(V_0-V_1)).flat[0],3)
    elif (r[' 0-3'] > 10) & (r[' 3-4'] < 2) & (r[' 3-5'] < 2) & (r[' 0-2'] < 2)  & (r[' 0-1'] >3) & (r[' 1-3'] >3):
        df.at[i,'type']= "type3-d"
        frag1=[0,2]
        frag2=[3,4,5]
        frag3=[1]
        KER_1=kinetic_energy_final(str(r['filename'])+"_final.xyz", frag1, masses)       # BOMD最後 frag1 
...
#        print(r['type'], r['KER'])
        ...
        df.at[i,"type"] = np.nan #"NAN"
        df.at[i,'KER'] =  np.nan #"NE"
#        print(df[i,'type'], r['KER'])
    i=i+1 
# version 2
